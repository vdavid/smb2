...
- `crates/smb2/` — the library, published as `smb2`. Most of this document is about it.
- `crates/smb2-cli/` — a command-line client built on it, published as `smb2-cli` (the installed binary is named
  `smb2`). Its own doc is `crates/smb2-cli/CLAUDE.md`; read that before touching the CLI.

The CLI depends on the library by `version` + `path`, so a library change and the CLI change that goes with it land in
one commit and get checked together.
...
- `just fuzz-seeds`: Regenerate the committed `crates/smb2/fuzz/corpus/` seeds

## Project structure

Everything shared sits at the repo root: `docs/`, `benchmarks/`, `justfile`, `rustfmt.toml`, `clippy.toml`,
`deny.toml`, `renovate.json`, the license files, `CHANGELOG.md`, `CONTRIBUTING.md`, `.github/`, and `.env`.

**Paths below, and everywhere else in this document, are relative to `crates/smb2/` unless they start with `crates/`.**
...
src/
  ...
  name.rs                 # Filename mapping for characters SMB2 forbids (private-use area)
  ...
  client/                 # High-level client API
    ...
    stream.rs             # FileDownload / FileReader / FileUpload / FileWriter
    read_ahead.rs         # ReadAhead + the adaptive pacing controller (`Window`) behind downloads AND uploads, its learned headroom
    tuning.rs             # The controller's tunables; `smb2::__bench` under the unstable `__bench-tuning` feature
    write_behind.rs       # WriteBehind (the upload knob), UPLOAD_CHUNK_SIZE, the one-frame write cut-off
    write_pipe.rs         # WritePipe: the one engine every pipelined upload runs on
    download_tests.rs     # FileDownload against the mock (order, cancel safety, closing, adaptive timing, rate hint)
    upload_tests.rs       # Uploads against the mock (chunk size, adaptive timing, fixed windows, rate hint, cancel safety)
...
fuzz/
  Cargo.toml              # Separate crate (nightly + libfuzzer-sys); cargo-fuzz wants it beside its crate
...
crates/smb2-cli/          # The CLI crate -- see its CLAUDE.md
...
they exercise, in each module's own `#[cfg(test)] mod tests`, not in `tests/`.
...
27. **A download's queue is latency for everything else on its connection** ✅ -- READs answer in order on one TCP stream, so a `stat` sent during a download waits behind every byte already queued: 23 s behind one 8 MB READ at 375 KB/s, 11.7 s behind eight 512 KiB READs. No fixed chunk or window is right on both a fast and a slow link, so `Tree::download` sizes its window to the link (`client/read_ahead.rs`), and `next_chunk` is cancel-safe because consumers drop it in `select!`. The window's margin (its headroom) is learned per connection from how late answers come, scored in arrival order so Samba's reordering doesn't read as jitter. A consumer choosing "one compound READ or stream?" asks `Connection::quick_read_limit`, which applies a fixed 250 ms frame budget, deliberately not the learned headroom. Uploads have the same problem the other way round (a `stat` waited 23 s behind an 8 MiB upload at 375 KB/s) and the same fix: every pipelined upload runs on `client/write_pipe.rs`, paced by the same `Window`, with `Connection::quick_write_limit` as the one-frame cut-off. See `client/CLAUDE.md` § Streaming download entry points and § Uploads.
...
29. **Whether a response is signed is not the server's to say** ✅ -- On a signed session every response is verified except the two MS-SMB2 § 3.2.5.1.3 exempts: an interim `STATUS_PENDING` and an unsolicited oplock break (MessageId `0xFFFF_FFFF_FFFF_FFFF`). ❌ Never gate verification on the header's `SMB2_FLAGS_SIGNED`: that bit travels in the same untrusted bytes as the payload, so an on-path attacker clears it and rewrites the response, and a client that believes the flag never checks. That is exactly what issue #5 demonstrated with a proxy rewriting a READ against Samba. The comparison is constant-time (`ctutils`), since `!=` stops at the first wrong byte. Mock tests follow from it: a mock session that isn't guest has signing on, so its canned unsigned responses are refused; `make_mock_client` declares a guest session for that reason. Spans `client/connection.rs` (`prepare_sub_frame`) + `crypto/signing.rs`.

30. **The response that turns signing on has to prove itself first** ✅ -- Pitfall 29 only protects a session that signs, and whether it signs (`IS_GUEST`, `IS_NULL`) or encrypts (`ENCRYPT_DATA`) is decided by the final SESSION_SETUP response, which arrives before any key is active, so `prepare_sub_frame` never checks it. An on-path attacker setting `IS_GUEST` there switches signing off for the whole session. `session.rs` `authenticate_final_response` runs after key derivation and before anything reads the flags: a login naming an account refuses a guest or anonymous session outright (MS-SMB2 § 3.2.5.3.1), then on SMB 3.x requires the response signed and verifies it with the new signing key (on 3.1.1 that key comes from the preauth hash, so it vouches for the negotiation too); on 2.x it verifies only a response flagged signed, since with guest refused there is nothing left there to downgrade. An empty username is a guest login the caller asked for and skips all of it; a login named `Guest` (any ASCII case, exactly) may be answered with a guest or anonymous session too, since that's what it asks for, and is verified like any account if the server grants a full one instead. Two things that are easy to get wrong: the success response is NOT in the preauth hash (pitfall 1), yet it IS what gets verified, with its own MessageId in the GMAC nonce; and a genuine guest response is never signed, so the guest check has to come first or a wrong password on a `map to guest = bad user` Samba reads as tampering. That Samba case is now a login error rather than a silent guest session (pinned against Docker Samba by `a_named_login_mapped_to_guest_is_refused`). Mock tests follow: `make_mock_client` logs in without a username because its mock grants a guest session, and `session.rs` tests derive the key as a server would (`auth::ntlm::exported_session_key_as_server`) to sign the final response for real. Spans `client/session.rs` + `crypto/signing.rs` + `auth/ntlm.rs` (test support).
...
```
crates/smb2/src/testing/CLAUDE.md   # Consumer test harness, TestServers API, embedded Docker infra
crates/smb2/src/client/CLAUDE.md    # SmbClient, Connection, compound, pipelining
crates/smb2/src/crypto/CLAUDE.md    # Signing, encryption, KDF, preauth hash
crates/smb2/src/msg/CLAUDE.md       # Wire format, Pack/Unpack, offsets, compounds
...
offline) so local clippy matches CI's always-latest stable, then runs `cargo fmt --all --check`,
...
```sh
RUST_LOG=smb2=debug cargo test -p smb2 --test integration -- --ignored
```
