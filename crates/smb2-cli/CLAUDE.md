...
End-to-end runs use the library's Samba fixtures, which are now in the same repo:

```sh
./crates/smb2/tests/docker/start.sh internal
SMB2_USER=testuser SMB2_PASS=testpass cargo run -p smb2-cli -- ls -l //127.0.0.1:10446/private
./crates/smb2/tests/docker/stop.sh
```

`smb-auth` is `testuser`/`testpass` on port 10446, share `private`. The other fixtures in `crates/smb2/tests/docker/internal/` cover signing, encryption, guest access, DFS, and flaky servers.
