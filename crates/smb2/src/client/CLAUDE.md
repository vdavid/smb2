...
| `read_ahead.rs` | `ReadAhead` (the public knob) and `Window`, the pure controller that decides when a transfer sends its next request, for downloads and uploads alike (`Pacing` is what both knobs map onto); `DOWNLOAD_CHUNK_SIZE`, `ADAPTIVE_MAX_IN_FLIGHT` |
...
- `Tree::download` reads 512 KiB chunks (`DOWNLOAD_CHUNK_SIZE`, capped at `MaxReadSize`) with `ReadAhead::Adaptive`: about `link rate × (RTT + 250 ms)` bytes still on their way, at least one READ, at most 4 MiB / 32 READs requested but undelivered. Chunks are delivered in file order from the front of `in_flight`; a short read re-requests its rest at the FRONT.
...
- **The tunables live in `read_ahead::Tuning`** (headroom, and `RateMeasure::LinkCapacity` vs the old `Deliveries` measurement), read through `Tuning::current()` by every window and both one-frame cut-offs, so a benchmark can compare candidates. The simulator in `read_ahead.rs`'s tests (`Link`) takes per-READ server delays (reordering, stalls) and per-chunk consumer delays through `with_server_delay` / `with_consumer_delay`.
- A download leaves its measured rate on the connection (`note_read_rate`, 30 s TTL, cleared on revival) and the next one starts from it, so a folder of 1 MiB files doesn't pay a round trip per file. Only a rate over 2+ answers is shared: one READ's is mostly its round trip.
- **Consumers read the same hint** through `Connection::download_rate_hint` (bytes/s) and `Connection::quick_read_limit`, the "one compound READ or stream?" cut-off: what the link moves in the same 250 ms headroom (`read_ahead::quick_read_limit`), at least one chunk, at most `MaxReadSize`. Keep the arithmetic there, so consumers don't each reinvent it.
...
- 512 KiB WRITEs (`UPLOAD_CHUNK_SIZE`, capped at `MaxWriteSize`, lowered by `fundable_chunk` per WRITE) and `WriteBehind::Adaptive`: about `upload rate × (RTT + 250 ms)` handed to the connection but unconfirmed, at least one WRITE, at most 4 MiB / 32 WRITEs. At 375 KB/s a `stat` waits 1.5 s at most during an upload, where the old 32 × `MaxWriteSize` window made it wait 23 s. ❌ Don't add a second write loop beside the pipe: the three that existed each had their own window, and a fix landed in one of them at a time.
...
- **`Connection::quick_write_limit`** is the one-frame cut-off: `max(512 KiB, upload rate × 250 ms)`, capped at `compound_write_limit()` (which wins over the floor, and can be 0). It never exceeds `compound_write_limit`, so a consumer that promised one frame by it can still send that frame after the limit moved.
