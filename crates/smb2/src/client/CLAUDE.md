...
- **The headroom is learned per connection** (`Window` § Learning the headroom): how late answers come against an ideal full pipe, scored in ARRIVAL order, into a windowed max (`read_ahead::Lateness`, 16 answers or 10 s, whichever reaches further back), clamped to 30–500 ms. Cold, it's 250 ms until 16 answers exist. Three things that look like simplifications and aren't:
  ...
  - ❌ **Don't shorten the memory to "a few answers".** A covered stall is invisible, so a memory that forgets it once it stops showing shrinks straight back under it; 16 answers are milliseconds on a fast link. The `span` in time is what holds it.
