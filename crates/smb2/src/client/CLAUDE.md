...
- A download leaves its measured rate on the connection (`note_read_rate`, 30 s TTL) and the next one starts from it, so a folder of 1 MiB files doesn't pay a round trip per file. Only a rate over 2+ deliveries is shared: one READ's is mostly its round trip.
