...
- `Tree::download` reads 512 KiB chunks (`DOWNLOAD_CHUNK_SIZE`, capped at `MaxReadSize`) with `ReadAhead::Adaptive`: about `rate × (RTT + 250 ms)` bytes still on their way, at least one READ, at most 4 MiB / 32 READs requested but undelivered. Chunks are delivered in file order from the front of `in_flight`; a short read re-requests its rest at the FRONT.
...
- The rate is taken over the last eight deliveries because Samba completes concurrent READs out of order; a per-gap rate swings with every burst. The RTT is NEGOTIATE's, capped by the fastest READ, so a slow NEGOTIATE (a waking NAS) can't budget seconds of queue.
- A download leaves its measured rate on the connection (`note_read_rate`, 30 s TTL, cleared on revival) and the next one starts from it, so a folder of 1 MiB files doesn't pay a round trip per file. Only a rate over 2+ deliveries is shared: one READ's is mostly its round trip.
...
- **What a confirmation measures**: the server having every byte, plus its disk and half a round trip. It reads low (answers share the downlink; a slow producer sees them late), never high, so the window errs small. The rate is shared on the connection apart from the download one (`note_write_rate`, `upload_rate_hint`, same 30 s TTL, cleared on revival), because most links are asymmetric.
