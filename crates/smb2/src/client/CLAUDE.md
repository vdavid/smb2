...
- ❌ **`next_chunk` must stay cancel-safe**; consumers drop it in `select!` loops. The head is awaited IN PLACE (`Connection::await_response_in_place`, the guard stays in the queue), `take_head` is synchronous, and a received chunk waits in `ready` across the awaits after it (next READs, CLOSE). Each has a test; the pre-fix code lost the last chunk to a dropped CLOSE, and the prototype lost whole chunks.
