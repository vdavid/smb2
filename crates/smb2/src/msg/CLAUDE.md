...
- **A DFS referral entry has three tails, and `DfsReferralEntry` is a sum type so the wrong one is unrepresentable.** ❌ Don't flatten it back into one struct. **V1** (§ 2.2.5.1) carries its `ShareName` *inline* at entry+8 with no offset, no `DFSPath`, and no TTL, so a V1 response is the one case where a cache key has to come from `PathConsumed` instead. **NameListReferral** (the `0x0002` entry flag, § 2.2.5.3.2) replaces the three path offsets with `SpecialNameOffset` + `NumberOfExpandedNames` + `ExpandedNameOffset`; read as a target it yields plausible-looking strings from wherever those bytes pointed, never an error. The expanded-name count is server-supplied and drives a loop, so the names are read one at a time and nothing pre-allocates for it.
- **`ServerType` is not a gate.** Samba answers a namespace-root referral with `ServerType = 0` and header flags `0x02` where Windows answers `1` and `0x03` (verified 2026-09-16). Store it; branching on it silently refuses every Samba namespace.
