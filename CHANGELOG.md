...
- **On a steady link, a listing waits less behind a download or upload, and on a link that stalls, transfers keep the pipe full.** The adaptive windows' margin on top of the round trip (the headroom) was a fixed 250 ms; it's now learned per connection and per direction from how late answers come, and kept for the next transfer the same way the rate is (30 s, cleared on reconnect). On a steady link it shrinks to 30 ms, so what a `stat` waits behind drops accordingly (40 ms instead of 64 ms at 50 MB/s in the simulator). Where answers stall or jitter it grows, up to 500 ms, and shrinks again once they stop. Answers Samba sends out of order aren't mistaken for jitter. `quick_read_limit` and `quick_write_limit` keep their fixed 250 ms budget. The parameters are provisional until the benchmark grid in `benchmarks/read-ahead/` confirms them.
