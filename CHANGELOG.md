...
- **A listing waits less behind a download or upload.** The adaptive read-ahead and write-behind windows keep a margin on top of the round trip (the headroom) so the pipe stays full, and every other request on the connection queues behind it. It was a fixed 250 ms. Now each connection learns it per direction from how late answers arrive, from 30 ms on a steady link up to 500 ms on a jittery one, and keeps it for the next transfer the same way it keeps the rate (30 s, cleared on reconnect). Answers Samba sends out of order, timestamps a few ms off, and TCP's slow restart after an idle spell don't read as jitter. Measured against Samba with a `stat` every 20 ms on the same connection (median wait during the transfer, against 0.25.1):
  - 30 MB/s, +1 ms, steady: 35 ms against 141, at the same throughput.
  - 3 MB/s uplink, +60 ms: 160 ms against 343. The windows now budget for the round trip of the quickest small request rather than NEGOTIATE's, which overstated it whenever a connection's first exchange was slow.
  - 30 MB/s, +20 ms with ±40 ms jitter: 107 ms against 225, and 24.7 MB/s against 16.9.
  - A real QNAP NAS over Wi-Fi: 61 ms against 92 during a 32 MiB download, at the same throughput. No other measurement there moved beyond noise.
  - The trade: a server that freezes now and then (a NAS disk seeking) is mostly left uncovered, so transfers run 4–7% slower through recurring 150 ms freezes, while a listing waits 52–108 ms through them instead of ~140.
- **`download_rate_hint` and `upload_rate_hint` report what the link can carry, so `quick_read_limit` and `quick_write_limit` stop streaming files that one frame moves faster.** They used to report the last transfer's pace, which a small window keeps well under the link's speed: a 4 MiB download at +60 ms on a ~1 GB/s link reported ~24 MB/s, so a consumer choosing by `quick_read_limit` streamed files a compound READ moves in half the time. The rate now comes from answers that queued behind each other on the wire, timed as they arrived, so a consumer catching up after a stall no longer reports a link several times faster than the real one either. It still errs low. The adaptive windows pace by the same rate, so they open faster on fast, distant links. `quick_read_limit` and `quick_write_limit` keep their fixed 250 ms budget.

Both changes come from a grid of 28 shaped links plus a real-NAS validation, written up in `benchmarks/read-ahead/results/self-tuning.md`.
- **A listing waits less behind a download or upload, on steady links and on ones that jitter or stall.** The adaptive windows' margin on top of the round trip (the headroom) was a fixed 250 ms; it's now learned per connection and per direction from how late answers come, and kept for the next transfer the same way the rate is (30 s, cleared on reconnect). On a steady link it shrinks to 30 ms, so what a `stat` waits behind drops accordingly: on a 30 MB/s link against Samba, 52–66 ms instead of 141 ms during a download, and 70 ms instead of 142 ms during an upload, at the same throughput. Where answers jitter it grows, up to 500 ms, and shrinks again once they stop. A server that freezes now and then (a NAS disk seeking) is mostly left uncovered: transfers run ~4–7% slower through recurring 150 ms freezes, and a listing waits 52–108 ms through them instead of ~140. Answers Samba sends out of order, answers timed a few ms off, and TCP's slow restart after an idle spell aren't mistaken for jitter. The round trip the windows budget for is now the quickest small request's rather than NEGOTIATE's alone, which overstated it whenever a connection's first exchange was slow; on a 3 MB/s uplink a `stat` behind an upload waits 161 ms where 0.25.1's waited 343. `quick_read_limit` and `quick_write_limit` keep their fixed 250 ms budget. The parameters come from a grid of 28 shaped links in `benchmarks/read-ahead/results/self-tuning.md`.

### Fixed

- **`write_file` flushes on Windows servers.** It sends CREATE, WRITE, FLUSH, and CLOSE in one frame, and Windows refuses a FLUSH that isn't last in a chain (`STATUS_INTERNAL_ERROR`), so a small file written to Windows was never flushed to disk, and nothing said so. Now the first write on a connection notices the refusal and flushes that file again, and later writes to that server end the chain on the FLUSH and close separately. That costs one extra round trip per file, on Windows only; Samba and NAS servers keep the one-round-trip write.

### Notes

- **The new `__bench-tuning` feature is internal.** It exposes a hidden `smb2::__bench` module that overrides the read-ahead tuning process-wide, for `benchmarks/read-ahead/`. It's unstable, outside SemVer, and can change or go away in any release, so don't enable it in an application.
...
- **Nobody on the network path can switch signing or encryption off at login.** Whether a session is signed and encrypted is decided by flags in the server's last SESSION_SETUP response, and the client believed them without checking. Setting `IS_GUEST` there turned signing off for the whole session, and clearing `ENCRYPT_DATA` turned encryption off, which reopened everything the fix above closes. Now a login with a username (NTLM or Kerberos) verifies that response's signature before reading its flags: on SMB 3.x it must be signed, as MS-SMB2 § 3.2.5.3.1 requires, and on SMB 2.x a signed one must verify. On SMB 3.1.1 a good signature also vouches for the whole negotiation, since the signing key comes from a hash of it. A missing or bad signature fails the login with `Error::Auth`.
- **A login with a username no longer quietly becomes a guest session.** When a server answers a named login with a guest or anonymous session, the login now fails with `Error::Auth` (`ErrorKind::AuthRequired`) saying so. That covers the tampering above, and it changes one thing you might notice: a Samba server set to `map to guest = bad user` answers a wrong password (or an unknown user) with a guest session, which used to connect you as guest with no signing and now reports the login as failed. To connect as guest on purpose, leave the username empty, which works as before.

### Changed

- **Uploads fill a fast link and stop blocking the connection on a slow one**, the way downloads have since 0.24.0. Every pipelined upload (`FileWriter`, `FileUpload` from `SmbClient::upload`, `Tree::write_file_pipelined`, `Tree::write_file_streamed`, `SmbClient::write_file_with_progress`) now writes 512 KiB WRITEs (`UPLOAD_CHUNK_SIZE`, capped at `MaxWriteSize`) with an adaptive write-behind window: about `upload rate × (RTT + 250 ms)` handed to the connection but not yet confirmed, at least one WRITE, at most 4 MiB. They used to keep up to 32 WRITEs of `MaxWriteSize` in flight (up to the connection's 32 MiB write budget), and every other request on the connection queued behind all of it. Measured against Samba with the uplink shaped (`benchmarks/read-ahead/results/adaptive-uploads.md`), 8 MiB uploads:
  - 375 KB/s: a `stat` on the same connection waits 1.5 s at most, down from 23.4 s, and a cancel takes 2.0 s, down from 6.1 s. Same total time.
  - 3 MB/s: a `stat` waits 370 ms at most, down from 2.9 s.
  - Fast links keep their throughput (100 MiB: 793 MB/s unshaped, against 637 before; 45.6 MB/s at +60 ms against 49.7; 16.0 at +200 ms against 16.6), and the worst `stat` at +200 ms drops from 1.8 s to 416 ms.
  - The next upload on a connection starts from the rate the last one measured. A cold connection sends one WRITE and opens the window once it's answered, so a small streamed file pays one extra round trip; `quick_write_limit` (below) is how a consumer avoids it.
  - To keep the old behavior on a `FileWriter`: `.with_chunk_size(max_write_size).with_write_behind(WriteBehind::Fixed(32))`.
- **`FileUpload::write_next_chunk` pipelines.** Each call hands one WRITE over and returns while the window has room, `bytes_written()` counts what the server has confirmed so far, and the call that sends the last chunk waits for every answer, flushes, and closes. It used to wait for each WRITE's answer before returning.
- **`SmbClient::write_file_with_progress` reports progress as the server confirms bytes**, and a cancel waits for the WRITEs already on the wire before closing the handle.

### Added

- **`WriteBehind`** (`Adaptive`, the default; `Fixed(n)`; `SEQUENTIAL`), and on `FileWriter`: `with_write_behind`, `with_chunk_size`, `write_behind()`, `chunk_size()`, and `peak_in_flight_bytes()`.
- **`UPLOAD_CHUNK_SIZE`** (512 KiB).
- **`Connection::quick_write_limit()`**: the largest file worth writing as one compound CREATE + WRITE + FLUSH + CLOSE on this connection right now; anything bigger streams better through a `FileWriter`. It's what the uplink moves in 250 ms, never less than one `UPLOAD_CHUNK_SIZE`, and never more than `compound_write_limit()` (which wins over the floor, and can be 0). On the bench, a 1 MiB file at +60 ms took 82 ms as one frame against 355 ms streamed, while at 375 KB/s it streams anything over one chunk, where one frame held every other request for 2.9 s.
- **`Connection::upload_rate_hint()`**: the rate, in bytes per second, that recent uploads on this connection had their WRITEs confirmed at, which `quick_write_limit` is built on. Measured apart from `download_rate_hint()` because most links are asymmetric. `None` until an upload of two or more WRITEs has run, 30 seconds after the last measurement, and after a reconnect.
...
- **`FileWriter::write_chunk` is cancel-safe.** Dropping it while it waited for room (a consumer's `select!`) lost the WRITE it was holding, and the next one landed at that WRITE's offset, so the file came out short with everything after the gap shifted. Now pushed bytes stay buffered until their WRITE is on the wire and go out with the next `write_chunk` or `finish`.
...
- **`Tree::read_file_compound_sized(conn, path, expected_size)`: a compound read that asks for the size you already know**, and pays credits for that instead of for a whole window. A server charges by *expected response* size (MS-SMB2 § 3.2.4.1.2), so the plain `read_file_compound`, which knows nothing about the file, asks for a full `MaxReadSize` and costs 130 credits on an 8 MiB server no matter what the file weighs. Against the 512-credit window that is three concurrent reads and no more: a 300 GB SMB-to-SMB copy that launched ten stalled with seven of them parked in `reserve_credits` behind `Create needs 130 credit(s) but only 0 are available`. A 4 MiB file read at its real size costs 66, so seven fit; a 4 KiB one costs 3, so 170 do. Callers with a directory scan in hand (most of them) get the concurrency back for one extra argument.
  - **`expected_size` is a bound, not a promise.** A file bigger than it fails with `Error::FileTooLargeForSingleRead` rather than coming back truncated, and the error carries the server's authoritative `size` so a caller can retry with it (when it still fits one READ; past `MaxReadSize` the answer is `read_file_pipelined`). This covers the file that grew between the scan and the read: the READ returns exactly `expected_size` bytes, which is indistinguishable from a complete read of a smaller file, so the guard compares the CREATE response's `end_of_file` against what was requested rather than against `MaxReadSize`. Same round-trip, no extra cost.
...
- **Overlapping connects to one server no longer wedge.** A pool of connections opened at once (`smb2-cli -j N`, a test binary running its cases in parallel, any consumer doing the same) could leave one worker hanging 30 s at connect and then failing it with `ServerUnresponsive`. It landed on a different worker each run, at a few percent, from four concurrent connects upward.
  ...
  - **It buys reliability, not speed.** Neither real server on hand (a QNAP TS-464, and a Pi on Samba 4.22.10) wedges at all, and on both, a shared guid costs nothing measurable: order-balanced A/B at `-j 16` puts the two builds within noise of each other. So the exposure is version-dependent, and the value here is that a pool of connections can't hang. Details, the log evidence, and how to measure it without fooling yourself: `docs/notes/samba-client-guid-connection-pass.md`.
...
- **A transfer costs the window, not the file.** `put` read the whole local file into memory before sending a byte, and `get` held the whole download. On a 600 MB file `put`'s peak resident set drops from 679 MB to 88 MB, and `get` runs in 58 MB where it previously could not run at all. Wall clock is unchanged (19.7 s vs 20.9 s on loopback), because both paths were already pipelined -- only where the bytes live changed.
  - A file that fits in one WRITE still goes up as a single compound CREATE+WRITE+FLUSH+CLOSE, so uploading something small is still one round trip. Downloads pay three round trips where they used to pay one, which is the price of not asking for a whole file at once; see the note on `commands/transfer.rs::Download::pump` for why trying the compound first is worse.
  - **A failed `get` leaves the destination exactly as it found it.** Streaming opens the destination while bytes are still arriving, so it takes care not to: the remote file is opened first, the bytes land on a `.smb2-part` sibling, and only a complete transfer renames it into place. Re-downloading a file you already have can no longer destroy your copy when the server says no.
...
- **A busy send queue no longer costs a diagnostic bundle its history.** Frames waiting for the socket were reported one `WARN` line each, every sweep. Copying 282 × 66 MB to a NAS over a 6.7 MB/s Wi-Fi link keeps around 320 one-MiB frames queued at all times, so that came to ~320 lines every 10 seconds: 4,487 of the 4,744 lines in one user's bundle were this single message, and the whole bundle covered 2 minutes 30 seconds. Everything that would have explained anything had already rotated out.
  ...
  - **The per-frame `send is slow` warning is gone with it.** It fired for any frame that spent 5 s queueing or writing, which on that link was every single frame -- another six or seven `WARN` lines a second, for sixteen minutes, about the same arithmetic. `SLOW_SEND_REPORT` is still the threshold for what counts as slow; it is now a count in the collapsed line rather than a line of its own.
