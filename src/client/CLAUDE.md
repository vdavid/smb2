...
- **Outstanding WRITE payload is bounded connection-wide** by `WRITE_BUDGET_BYTES` (32 MiB), a second `Semaphore` alongside the credit pool. `MAX_PIPELINE_WINDOW` bounds ONE stream, so N concurrent uploads multiply it: ten files at a 1 MiB `MaxWriteSize` reached 320 MiB queued in-process, ~48 s of latency on a 6.7 MB/s link. The permit rides INSIDE the in-flight WRITE future, so drain, abort, cancel, and drop all return it for free. ❌ Never hold it on the writer.
- **❌ Never await the write budget while holding a permit you aren't polling** -- the one way to deadlock it. `FileWriter::reserve_budget` takes budget only if free, otherwise drains its OWN response first, and waits only when it holds nothing.
- **`Tree::write_file_pipelined` / `write_file_streamed` are NOT budgeted yet.** They pipeline `MAX_PIPELINE_WINDOW` the same way and carry the same multiplication; nothing in Cmdr reaches them (it goes through `FileWriter`), which is the only reason this is a gap and not a bug.
